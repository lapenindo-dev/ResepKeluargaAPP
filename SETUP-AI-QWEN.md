...
4. Pilih sumber: `Internet`, `Warisan`, `Keluarga`, `Teman`, atau `Kreasi sendiri`.
5. Klik **Rapikan dengan AI**.

Tidak ada dependency tambahan untuk YouTube di `package.json`.


## Login Email / Proteksi Akses v2.3.5
...
Catatan: policy SQL v2.3.5 membuat hanya user yang sudah login yang bisa baca/tambah/edit/hapus data. Semua user login keluarga berbagi data resep yang sama.
...
## v2.3.5 - Backup JSON disembunyikan dari user keluarga
...
## Update v2.3.5 - Penulis dan Last Edit

Jalankan SQL baru berikut di Supabase SQL Editor:

`supabase-migration-v2.3.5-author-timestamps.sql`
...
- Halaman tambah/edit resep otomatis mulai dari bagian paling atas.
- Tombol Simpan ditambahkan di bagian atas dan tetap ada di bawah.
...
- Input Resep disederhanakan menjadi 2 cara: **Input Manual** dan **Foto / Caption**.
- Foto Utama diberi border putih halus, outline tipis, radius, dan shadow agar tidak terlalu mepet ke lebar device.


## Update v2.3.5 - Sumber Resep

- Opsi sumber resep disederhanakan menjadi: Internet, Warisan, Keluarga, Teman, Kreasi sendiri.
- Data lama tetap kompatibel: Manual ditampilkan sebagai Kreasi sendiri; YouTube/AI ditampilkan sebagai Internet.
- Tidak perlu SQL migration baru.
